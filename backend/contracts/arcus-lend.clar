...
;; data maps and vars
(define-data-var bridge-money int 21)  ;;this token work as a floating money can be used as lending a loan
...
(define-public (add-demo-user (address principal) ) 
    (begin  
...
(define-public (send-loan (receiver-address principal) (amount int)) 
    (begin  
        (asserts! (is-eq tx-sender contract-owner) err-owner-only) ;;check for the owner priviledge
        (asserts! (>=  (var-get bridge-money) amount)  err-not-enough-money) ;;token money > amount to be able to send loan
